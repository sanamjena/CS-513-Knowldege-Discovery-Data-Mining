...
#  Project    : CS-513 B_Homework 8_2 
#  Purpose    : Using k-means, categorize the "wisc_bc_ContinuousVar.csv" data into two (2) clusters based on. 
...
#################### Applying kmeans & Plotting #############################
Breast_Cancer <- Breast_Cancer[-1]
cancerdist <- dist(Breast_Cancer[,-1])
kmeanss <- kmeans(Breast_Cancer[,-1],2,nstart = 10)
kmeanss$cluster
table(kmeanss$cluster,Breast_Cancer[,1])